...
Ce chapitre est consacré à la conception et à l’architecture de la plateforme CSR. Il présente les exigences fonctionnelles et non fonctionnelles du projet, ainsi que l’organisation de la gestion du projet selon la méthodologie Scrum, à travers le product backlog, les sprints et la chronologie des travaux. Il aborde également la modélisation du système à l’aide des diagrammes UML\footnote{UML : \emph{Unified Modeling Language} (langage de modélisation unifié). Il s’agit d’un ensemble de notations graphiques standardisées pour représenter, analyser et concevoir des systèmes logiciels orientés objet (cas d’utilisation, classes, séquences, etc.).}. Enfin, il présente les choix technologiques ainsi que les environnements matériel et logiciel utilisés pour le développement de la plateforme.
...
\begin{itemize}[label=\textbullet, itemsep=0.15ex, parsep=0pt, topsep=0pt, partopsep=0pt]
  \item Créer des plans annuels CSRavec gestion des statuts (brouillon, soumis, validé, rejeté, etc.).
  \item Prendre en charge les modes de validation (chaîne de validation adaptée aux processus par site/plan).
  ...
  \item Intégrer la gestion des activités hors plan.
  \item Gérer le cycle de vie des plans et des activités (modification, soumission, validation, rejet, verrouillage).
  ...
  \item Assurer l’approbation ou le rejet par les acteurs corporate, avec validation multi-niveaux.
  \item Verrouiller le plan après validation définitive.
  \item Importer des fichiers Excel consolidés avec prévisualisation, contrôles (région/pays/site), détection de conflits et paramétrage par site/plan.
  \item Calculer et suivre les indicateurs clés de performance (KPI) de réalisation du plan (taux d’exécution, budget consommé...).
\end{itemize}

\textbf{Gestion des activités CSR}

\begin{itemize}[label=\textbullet, itemsep=0.15ex, parsep=0pt, topsep=0pt, partopsep=0pt]
  \item Planifier les activités CSR avec saisie des données prévisionnelles.
  \item Enregistrer la réalisation des activités planifiées via des enregistrements (records) datés, incluant les valeurs réelles.
  \item Ajouter des activités hors plan et permettre leur soumission/validation avec les mêmes règles de traçabilité.
  \item Autoriser la modification des activités et des réalisations tant qu’elles sont dans un état modifiable, puis verrouiller après validation.
  \item Associer des pièces justificatives (documents, photos, rapports) aux activités et aux réalisations pour faciliter l’audit.
  \item Calculer automatiquement les KPI d’exécution à partir des données planifiées et réalisées.
  \item Assurer le suivi des écarts planifié/réalisé et la mise à jour des indicateurs après chaque création ou modification de record.
\end{itemize}
...
\begin{itemize}[label=\textbullet, itemsep=0.15ex, parsep=0pt, topsep=0pt, partopsep=0pt]
  ...
  \item Calculer les taux kpi des activités.
...
\input{tables/chapitre-02-product-backlog}
...
\input{tables/chapitre-02-tab-gimsi-phases}
...
\input{tables/chapitre-02-planification-sprints}
...
\begin{landscape}
  ...
  % \thispagestyle suffit ici: \pagestyle{empty} désactive l'en-tête globalement.
  \thispagestyle{empty}
